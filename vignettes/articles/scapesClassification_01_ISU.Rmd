...
The following is an example of how to use [scapesClassification](https://github.com/ghTaranto/scapesClassification) to classify marine seafloor structures, hereafter referred to as geomorphic management units (GMUs). Our study area is located in the 'Triangle' of the Azores (NE Atlantic), around the islands of Faial, Pico and São Jorge ([Figure 1](#figure1)). 

The GMU-classification will be presented in three separate articles:

1. This article presents the general workflow of a classification process implemented with _scapesClassification_ ([Geomorphic management units](scapesClassification_01_ISU.html)). 

2. The second article presents a strategy to identify _local maxima_ on gridded data ([Local maxima on gridded data](scapesClassification_02_RU.html)). 

3. The third article presents a strategy to identify independent objects on raster surfaces ([Independent objects on raster surfaces](scapesClassification_03_RU.html)). 


# Load data and libraries
Geomorphic management units (GMUs) will be identified using a [`RasterStack`](https://rspatial.org/raster/spatial/4-rasterdata.html#rasterstack-and-rasterbrick) of six layers that includes bathymetry, bathymetric derivatives ([Walbridge et al., 2018](https://www.mdpi.com/2076-3263/8/3/94/htm)) and the geomorphons of the area ([Jasiewicz and Stepinski, 2013](https://www.sciencedirect.com/science/article/pii/S0169555X12005028#!)):
...
__`anchor.svo()`:__ returns a vector of cell numbers extracted at the locations of a spatial vector object. In this example, _'land cells'_ are defined as: _(i)_ raster cells extracted at island locations that are not complete cases (arg. `ONLY_NA = T`); _(ii)_ cells adjacent to cells of point _(i)_ that are not complete cases either (arg. `fill_NAs = T`) ([Figure 2](#figure2)). 
...
__`anchor.cell()`:__ converts a vector of cell numbers into a **class vector**. In this example, all cells adjacent to _'land cells'_ (arg. `class2cell = F` and `class2nbs = T`) will be considered as _'ISU-anchor cells'_ ([Figure 2](#figure2)).
...
```
_FIGURE 2 - **Anchor cell:** raster cells adjacent to a 'land cell'. **Land cell**: raster cells extracted at the locations of the 'island shapefile' that are not complete cases and all adjacent cells that are not complete cases either. **Complete cases**: raster cells having no missing values in no layer of the stack ([Figure 1](#figure1)); note that no overlap exists between "land cell" and "complete cases". **Island shapefile**: the shapefile of Faial, Pico and São Jorge Islands._
...
__`cond.4.nofn()`:__ Evaluate conditions for cells neighboring specific classes. In this case, the function identifies the _'flat cells'_ of island shelves based on the above set of rules ([Figure 3](#figure3)).
...
Once _'flat cells'_ have been identified, we will assume that raster cells surrounded by _'island shelf cells'_ are also _'island shelf cells'_. We define a cell as surrounded if 60% of its neighbors belong to an 'island shelf' class (_'hole cell'_, [Figure 3](#figure3)). 
...
The class vector `slope3` includes all ISU-cells ([Figure 4](#figure4)). It can be saved as an `*.RDS` file and used in the next article ([Local maxima on gridded data](scapesClassification_02_RU.html)). 
