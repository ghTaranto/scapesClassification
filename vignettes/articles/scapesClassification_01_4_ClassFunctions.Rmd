...
<div style="line-height: 0.8em; margin-bottom: 1.5em;"><span class="legend">**Figure 3 -** **Anchor.cell**, converts a vector of cell numbers into a class vector. **In blue**, cells included in the class vector using the cell number vector `1:7` as a function input. The plotted values correspond to raster cell numbers.</span></div>
...
The function `cond.4.nofn()` can be used to evaluate both relative and absolute conditions for cells neighboring specific classes (see `?conditions`, [classificaton rules](scapesClassification_01_3_ClassRules.html) and [focal evaluation](scapesClassification_01_2_RuleEvaluation.html#focal-evaluation)).  
...
We can now use the function `cond.4.nofn()` to test an absolute test cell condition `"dummy_var >= 3"` at locations adjacent to the focal cell ([Figure 6](#figure6)).
