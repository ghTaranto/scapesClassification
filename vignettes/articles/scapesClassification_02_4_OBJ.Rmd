...
m@map %>% 
  hideGroup(c("Depth (m)", "RDG", "Local BPI", "Peak")) %>% 
  addLayersControl(overlayGroups = c("RO", "RDG", "Peak", "Depth (m)", "Local BPI"),
                   position = "topleft",
                   options = layersControlOptions(collapsed = FALSE))
```

<div style="line-height: 0.9em; margin-bottom: 1.5em;"><span class="legend">**Figure 4 - Raster object interactive map,** colors identify discrete raster objects. **RO**, clickable layer. **RDG**, ridges within raster objects. **Peaks**, main peak of each raster object. Bathymetry and local BPI values are provided in background (due to plot re-projection, displayed and original values might be slightly different).</span></div>

## Final class vector

The relief unit class vector (which is composed by all the raster objects found) is saved in the file `"RU_obj.RDS"` and shared as part of the package data. 
