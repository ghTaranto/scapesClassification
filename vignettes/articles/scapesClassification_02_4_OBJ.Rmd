---
title: "4.Independent objects on raster surfaces"
author: "Gerald H. Taranto"
date: "`r format(Sys.time(), '%a %d, %B, %Y')`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{4.Independent objects on raster surfaces}
...
In this article we will classify prominent seamounts, banks and ridges as individual raster objects ([Figure 2](#figure2)) each characterized by a unique ID (or class). In a first phase, we will identify the core cells of independent relief unit objects (RU-objects) ([Figure 1](#figure1)). In a second phase, we will refine their borders ([Figure 2](#figure2)). 

## 4.1 Load data and libraries
...
# RU CORE, STEP 1
atbl$RU_core <- cond.4.nofn(atbl, nbs, rNumb = TRUE, 
...
atbl$RU_obj <- anchor.seed(
  ...
  attTbl = atbl, ngbList = nbs, rNumb = TRUE,  silent = TRUE,
...
```
<div style="line-height: 0.8em; margin-bottom: 1.5em;"><span class="legend">**Figure 1 -** Core_ST1, core cell type1; Core_ST2, core cell type2; Rm_IDs, RU_obj removed because of their little relevance at the scale of our study; RU_obj, relief unit objects (clickable layer).</span></div>


## 4.3 RU-objects: refine borders
...
while (ITR<15) {
  ...
  for(i in RU_IDs){
    ...
    atbl$RU_obj2 <- cond.4.nofn(atbl, nbs, rNumb = TRUE, 
...
m@map %>%
  addLayersControl(overlayGroups = c("RU_obj2"),
                   ...
                   options = layersControlOptions(collapsed = FALSE))
```
<div style="line-height: 0.8em; margin-bottom: 1.5em;"><span class="legend">**Figure 2 -** RU_obj2, relief unit objects with refined borders (clickable layer).</span></div>

## 4.4 RU-objects: class vectors
