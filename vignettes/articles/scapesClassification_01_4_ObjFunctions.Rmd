...
Note that the computed class vectors are added to the attribute table, in this way classification rules can take them into account. 
...
The function `obj.border()` identifies the borders of raster objects. To handle missing values on raster surfaces, the function was designed to work with raster cells identified by attribute table row indices rather then with cell numbers (see [format inputs](scapesClassification_01_1_FormatInputs.html)). When there are no missing values, row cell indices and cell numbers are equivalent (i.e. n^th^ row correspond to the n^th^ cell). With missing values it ceases to be true. 

As an example we will re-compute the class vector `cv2` of the previous example, but before we will add an NA-value in the initial raster and compute the neighborhood list with neighborhoods identified by row numbers (see `?ngbList`).
...
We can re-compute the [raster objects](scapesClassification_01_4_ObjFunctions.html#hgrowth).

```{r obj_2, message=FALSE, warning=FALSE, echo=TRUE, collapse=TRUE}
# a. cv0, identify peak cells
at$cv0 <- peak.cell(at, nbs, rNumb=TRUE, p_edge=TRUE, p_col="dummy_var", p_fun="max")

# b. cv1, assign unique IDs
at$cv1 <- anchor.seed(at, nbs, rNumb=TRUE, silent=TRUE, cond.filter="!is.na(cv0)", cond.seed="TRUE")

# c. cv2, homogeneous growth of raster objects                   
at$cv2 <- cond.4.nofn(at, nbs, rNumb=TRUE, classVector=at$cv1, nbs_of=c(1, 2), hgrowth=TRUE,
                      cond="dummy_var <= dummy_var[] & dummy_var > 1")
```

And identify their borders ([Figure 4](#figure4)).
...
RO_bd1 <- at$Cell[RO_bd[["1"]]]
RO_bd2 <- at$Cell[RO_bd[["2"]]]
...
The function `obj.nbs()` identifies the shared borders of raster objects as well as all the neighbors of every raster object. As in the case of the function [obj.border](scapesClassification_01_4_ObjFunctions.html#obj-border), `obj.nbs()` was designed to work with raster cells identified by attribute table row indices (see [format inputs](scapesClassification_01_1_FormatInputs.html)). When there are no missing values, row cell indices and cell numbers are equivalent (i.e. n^th^ row correspond to the n^th^ cell). With missing values it ceases to be true. 

The function `obj.nbs()` returns a named nested lists:
...
RO_bd <- obj.border(RO, nbs, silent = TRUE) # compute borders

# COMPUTE SHARED BORDERS
RO_shared_bd <- obj.nbs(RO_bd, nbs, silent = TRUE)

# Obj.1 has as neighbor Obj.2 and its border with it are cells in rows 24 and 31
RO_shared_bd[["1"]]

# Obj.2 has as neighbor Obj.1 and its border with it are cells in rows 30 and 37 
RO_shared_bd[["2"]]

# CONVERT ROW INDICES TO CELL NUMBERS
sharedBD1 <- at$Cell[ RO_shared_bd[["1"]][["2"]] ]
sharedBD2 <- at$Cell[ RO_shared_bd[["2"]][["1"]] ]
```
