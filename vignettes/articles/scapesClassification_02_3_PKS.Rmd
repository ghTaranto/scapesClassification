...
```
_Figure 1 - PSAs, peak search areas; PSAs5, peak search areas with less than 5 raster cells excluded from the PSA filter; ISU, island shelf unit._
...
m@map %>% 
  addLayersControl(overlayGroups = c("PSA", "PEAKS"),
                   ...
                   options = layersControlOptions(collapsed = FALSE))
```
_Figure 2 - Peaks and peak search areas. Note that there is a peak within each peak search area. In background are provided depth values; because of reprojection displayed and original depth values are slightly different._
